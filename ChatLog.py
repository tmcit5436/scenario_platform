...
class Chat(tk.Frame):
    ...
    def send_msg(self, msg: str, sender=('sender','red'), side='left', color='white'):
        ...
        self.msg_index += 1
        self.canvas.config(scrollregion=(0,0,0,self.y_diff[-1]))
        self.canvas.yview_moveto(1)
    def send_left_msg(self, msg, sender, color, tag):
        # 名前
        if not sender is None:
            name_id = self.canvas.create_text(15, self.y_diff[-1]-3, text=sender[0], font=('',11), fill='black', anchor=tk.NW, tags=tag)
            pos = self.canvas.bbox(name_id)
            self.y_diff.append(self.y_diff[-1] + pos[3] - pos[1])
        # メッセージ本文
        text_id = self.canvas.create_text(20, self.y_diff[-1], text=msg, fill='black', width=250, anchor=tk.NW, tags=tag)
        ...
        self.y_diff.append(self.y_diff[-1] + pos[3] - pos[1] + 10)
    def send_right_msg(self, msg, sender, color, tag):
        # 名前
        if not sender is None:
            name_id = self.canvas.create_text(self.width-12, self.y_diff[-1]-3, text=sender[0], font=('',11), fill='black', anchor=tk.NE, tags=tag)
            pos = self.canvas.bbox(name_id)
            self.y_diff.append(self.y_diff[-1] + pos[3] - pos[1])
        # メッセージ本文
        text_id = self.canvas.create_text(self.width-17, self.y_diff[-1], text=msg, fill='black', width=250, anchor=tk.NE, tags=tag)
        pos = [elem+diff for elem, diff in zip(self.canvas.bbox(text_id), (-2,-2,2,2))]
        # 吹き出し
        back_id = self.canvas.create_polygon(
